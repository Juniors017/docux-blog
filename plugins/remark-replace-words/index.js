...
function isForbiddenParent(parent, replacementComponentNames) {
  ...
  if (
    ...
    (parent.name === "a" || replacementComponentNames.has(parent.name))
  ) {
    return true;
  }
...
export default function remarkReplaceFromJson() {
  return (tree, file) => {
    ...
    const entries = Object.entries(mergedMapping).sort(
      ...
      for (const [word, conf] of entries) {
        ...
        fragments = fragments.flatMap((frag) => {
          ...
          if (DEBUG) {
            ...
            stats[filePath][word] =
              (stats[filePath][word] || 0) + matches.length;
          }
            ...
            if (i < matches.length) {
              ...
              if (compName) {
                ...
                newNodes.push({
                  type: nodeType,
                  name: compName,
                  attributes: Object.entries(conf.props || {}).map(
                    ([key, value]) => ({
                  ...
                  children: replacementLabel
                    ? [{ type: "text", value: replacementLabel }]
                    : [],
                });
              } else {
    ...
    if (
      ...
      !global.__remarkReplaceWordsReported
    ) {
      ...
      setTimeout(() => {
        ...
        for (const [file, words] of Object.entries(stats)) {
          ...
          for (const [word, count] of Object.entries(words)) {
            if (
              mapping.all &&
              Object.prototype.hasOwnProperty.call(mapping.all, word)
            ) {
