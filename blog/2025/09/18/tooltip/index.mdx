...
![Docux frowning at a screen, surrounded by floating speech bubbles asking Variable? and Syntaxe?](/img/tooltipcomponent.webp)

</div>
</center>

<Tooltip model="teacher" text="Tooltips">
  In computing, a tooltip is a message that appears as an overlay when the mouse
  pointer hovers over certain elements of the graphical user interface. It gets
  its name from its resemblance to speech bubbles in comic strips [Source
  Wikipedia](https://en.wikipedia.org/wiki/Tooltip)
</Tooltip>
are everywhere on the web. They are essential for providing contextual
information without cluttering the interface. But creating one that is robust,
aesthetically pleasing, and above all, accessible, is a real challenge.

I developed this `Tooltip` component for <Tooltip model="teacher" text="Docusaurus" >Docusaurus is a static-site generator.
...
Before diving into the demo, here's an overview of the features I integrated to make this component cool.
...
<Trees>
  <Folder label="src" expanded={true}>
    <Folder label="components" expanded={true}>
      <Folder label="Tooltip" expanded={true}>
        <File label="index.js" icon="javascript" />
        <File label="styles.module.css" icon="logos:css-3" />
        <File label="models.js" icon="javascript" />
      </Folder>
    </Folder>
    <Folder label="theme" expanded={true}>
...
<Snippet
  filename="/src/components/Tooltip/index.js"
  source="src/components/Tooltip/index.js"
/>

<Snippet
  filename="/src/components/Tooltip/styles.module.css"
  source="src/components/Tooltip/styles.module.css"
/>

## Models Code

The `models.js` file contains **predefined styles** to facilitate reusability and consistency of tooltips in your application. Instead of redefining the same styles each time, you can import and use these models.

### Model Advantages

- **Reusability**: Define once, use everywhere
- **Consistency**: All tooltips of the same type have the same style
- **Maintenance**: Modify the style in a single place
- **Simplicity**: No need to memorize colors and styles

### Available Models

The file offers several ready-to-use models:

- **`info`**: Blue tooltip (#2196F3) for information
- **`success`**: Green tooltip (#4CAF50) for success messages
- **`warning`**: Yellow tooltip (#FFC107) for warnings
- **`error`**: Red tooltip (#F44336) for errors
- **`custom`**: Function to create your own custom styles
- **`teacher`**: Special model with background image and custom border
...
```jsx
<Tooltip text="Message" backgroundColor="#2196F3" color="#FFFFFF">
  Hover me
</Tooltip>
```

You can simply import the model and use it:

```jsx
<Tooltip text="Message" Type="info">
  Hover me
</Tooltip>
```

<Snippet
  filename="/src/components/Tooltip/models.js"
  source="src/components/Tooltip/models.js"
/>
...
```mdx
<Tooltip model="teacher" text="DocuxLab">
  My favorite and the one you will find mostly on this site. We can really make
  nice tooltips, it dusts off the classic info bubble.
  <LogoIcon name="docusaurus" size="20" />
</Tooltip>
...
You can even include other components, and even an image.
  ...
  <center>
  ![Docux wearing sunglasses](/img/tooltipstest.webp)
  </center>
</Tooltip>
...
<div style={{ display: "flex", gap: 16 }}>
  <Tooltip model="teacher" text="DocuxLab">
    My favorite and the one you will find mostly on this site.
    <br />
    We can really make nice tooltips, it dusts off the classic info bubble.
    <LogoIcon name="docusaurus" size="20" />
  </Tooltip>
