...
![Docux and a purple goat beside a casino slot machine displaying 100k views](/img/goatcounterviews.webp)
...
<Trees>
  <Folder label="src" expanded={true}>
    <Folder label="components" expanded={true}>
      <Folder label="GoatCountViews" expanded={true}>
...
<Snippet
  filename="src/components/GoatCountViews/index.js"
  source="src/components/GoatCountViews/index.js"
/>
...
If you display _multiple_ posts, you must request the counter for **each post** by passing its exact path. For example, in a swizzled component (header of a blog post item):
...
![A post header showing the date, the reading time and a view counter reading 9](/img/vuedesviewgoat.webp)
